...
{
    internal sealed class Vns430PanelArtwork : IDisposable
    {
        ...
        private Image Load(string relative)
        {
            ...
            Assembly assembly = typeof(Vns430PanelArtwork).Assembly;
            string suffix = ".VNS430.Assets." + relative.Replace('/', '.').Replace('\\', '.');
            ...
            if (resource == null)
            {
                throw new FileNotFoundException("Embedded VNS430 artwork was not found.", relative);
            }

            using Stream stream = assembly.GetManifestResourceStream(resource);
            using Image source = Image.FromStream(stream ?? throw new FileNotFoundException("Embedded VNS430 artwork could not be opened.", resource));
